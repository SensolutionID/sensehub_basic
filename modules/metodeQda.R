  ...
  tagList(
    fluidRow(
      column(
        4,
        helper(
          h1("Quantitative Descriptive Analysis"),
          type = "markdown",
          content = "metodeQda",
          size = "l"
        ),
        wellPanel(
          ...
          conditionalPanel(
            ...
            tipify(
              ...
              title = "Harus berupa data kontinyu! Contoh: kesukaan.", placement = "top"
            ),
            actionButton(
              inputId = ns("terapkan"),
              label = "Terapkan"
...
metodeQda <- function(input, output, session) {
  ns <- session$ns
  ...
  mentah <- eventReactive(input$unggah, {
    if (input$sumber_data == "contoh") {
      read_csv("data/perfumes_qda_consumers.csv")
  ...
  ringkasan <- eventReactive(input$terapkan, {
    ...
    {
      tibble(
        "Metode" = "Quantitative Descriptive Analysis",
  ...
  grafik_atributUtama <- reactive({
    ...
    nilai_atributUtama() %>% 
      ...
      labs(x = "", y = "Rerata", fill = "", title = input$opsi_grafik_atributUtama) +
      guides(fill = FALSE) +
      theme_ipsum() +
      theme(
  ...
  global <- eventReactive(input$terapkan, {
    req(nilai_atributUtama())
    if (is.null(input$atribut_tambahan)) {
      df <- nilai_atributUtama() %>%
        select(Atribut, Sampel, Rerata) %>% 
        spread(key = "Atribut", value = "Rerata") %>% 
        as.data.frame() %>% 
        `rownames<-`(.[,"Sampel"]) %>% 
        select_if(is.numeric)
      kuanti_tambahan <- NULL
    } else {
      df <- bind_rows(nilai_atributUtama(),
                      nilai_atributTambahan()) %>% 
        select(Atribut, Sampel, Rerata) %>% 
        spread(key = "Atribut", value = "Rerata") %>% 
        select(Sampel, one_of(input$atribut, input$atribut_tambahan)) %>% 
        as.data.frame() %>% 
        `rownames<-`(.[,"Sampel"]) %>% 
        select_if(is.numeric)
      kuanti_tambahan <- which(names(df) %in% input$atribut_tambahan)
    }
    res <- PCA(df, quanti.sup = kuanti_tambahan, graph = FALSE)
    return(res)
  })
  ...
  global_dimensi <- reactive({
    if (is.null(input$atribut_tambahan)) {
      ...
      list(
        ...
        "Atribut Tambahan" = list(
          Koordinat = global()$quanti.sup$coord %>%
            signif(2) %>%
            `colnames<-`(paste("Dim", 1:ncol(.))),
          Korelasi = global()$quanti.sup$cor %>%
            signif(2) %>%
            `colnames<-`(paste("Dim", 1:ncol(.)))
        )
      )
